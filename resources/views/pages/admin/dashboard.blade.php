<x-app-layout>


    <div class="flex flex-wrap  px-6 mx-0 md:mx-4 mb-4 md:mb-8 md:mb-6 md:mt-4">
        <div class="w-full px-6 py-8 bg-gradient-to-r from-blue-200 to-blue-300 rounded shadow">
            <h2 class="font-bold text-lg md:text-3xl">Hello, Good Morning 👋</h2>
            <h5 class=" text-sm md:text-base">here is whats happening:</h5>
        </div>
    </div>


    <div class="flex flex-wrap px-0 md:px-4 ">

        <div class="w-full md:w-1/2 xl:w-1/4 px-6 pb-4 ">
            <div
                class="bg-gradient-to-b from-yellow-200 to-yellow-100 border-b-4 border-yellow-600 rounded p-5 shadow-md hover:shadow-xl transition delay-75">
                <div class="flex flex-row items-center">
                    <div class="flex-shrink pr-4">
                        <div class="rounded-full p-5 bg-yellow-600"><i class="fas fa-user-plus fa-2x fa-inverse"></i>
                        </div>
                    </div>
                    <div class="flex-1 text-right md:text-center">
                        <h5 class="font-bold uppercase text-gray-600">New Users</h5>
                        <h3 class="font-bold text-3xl">2 <span class="text-yellow-600"><i
                                    class="fas fa-caret-up"></i></span></h3>
                    </div>
                </div>
            </div>
        </div>

        <div class="w-full md:w-1/2 xl:w-1/4 px-6 pb-4 ">
            <div
                class="bg-gradient-to-b from-yellow-200 to-yellow-100 border-b-4 border-yellow-600 rounded p-5 shadow-md hover:shadow-xl transition delay-75">
                <div class="flex flex-row items-center">
                    <div class="flex-shrink pr-4">
                        <div class="rounded-full p-5 bg-yellow-600"><i class="fas fa-user-plus fa-2x fa-inverse"></i>
                        </div>
                    </div>
                    <div class="flex-1 text-right md:text-center">
                        <h5 class="font-bold uppercase text-gray-600">New Users</h5>
                        <h3 class="font-bold text-3xl">2 <span class="text-yellow-600"><i
                                    class="fas fa-caret-up"></i></span></h3>
                    </div>
                </div>
            </div>
        </div>

        <div class="w-full md:w-1/2 xl:w-1/4 px-6 pb-4 ">
            <div
                class="bg-gradient-to-b from-yellow-200 to-yellow-100 border-b-4 border-yellow-600 rounded p-5 shadow-md hover:shadow-xl transition delay-75">
                <div class="flex flex-row items-center">
                    <div class="flex-shrink pr-4">
                        <div class="rounded-full p-5 bg-yellow-600"><i class="fas fa-user-plus fa-2x fa-inverse"></i>
                        </div>
                    </div>
                    <div class="flex-1 text-right md:text-center">
                        <h5 class="font-bold uppercase text-gray-600">New Users</h5>
                        <h3 class="font-bold text-3xl">2 <span class="text-yellow-600"><i
                                    class="fas fa-caret-up"></i></span></h3>
                    </div>
                </div>
            </div>
        </div>

        <div class="w-full md:w-1/2 xl:w-1/4 px-6 pb-4 ">
            <div
                class="bg-gradient-to-b from-yellow-200 to-yellow-100 border-b-4 border-yellow-600 rounded p-5 shadow-md hover:shadow-xl transition delay-75">
                <div class="flex flex-row items-center">
                    <div class="flex-shrink pr-4">
                        <div class="rounded-full p-5 bg-yellow-600"><i class="fas fa-user-plus fa-2x fa-inverse"></i>
                        </div>
                    </div>
                    <div class="flex-1 text-right md:text-center">
                        <h5 class="font-bold uppercase text-gray-600">New Users</h5>
                        <h3 class="font-bold text-3xl">2 <span class="text-yellow-600"><i
                                    class="fas fa-caret-up"></i></span></h3>
                    </div>
                </div>
            </div>
        </div>



    </div>

    <div class="flex flex-row flex-wrap flex-grow">

        <div class="w-full  xl:w-1/3 p-6 lg:pl-10">
            <div class="bg-white h-96 rounded p-5 shadow-md hover:shadow-xl transition delay-75">

            </div>
        </div>

        <div class="w-full  xl:w-2/3 p-6 lg:pr-10">
            <div class="bg-white h-96 rounded p-5 shadow-md hover:shadow-xl transition delay-75">

            </div>
        </div>
    </div>

    <div class="flex flex-row flex-wrap flex-grow">

        <div class="w-full  xl:w-1/2 p-6 lg:pl-10">
            <div class="bg-white h-96 rounded p-5 shadow-md hover:shadow-xl transition delay-75">

            </div>
        </div>

        <div class="w-full  xl:w-1/2 p-6 lg:pr-10">
            <div class="bg-white h-96 rounded p-5 shadow-md hover:shadow-xl transition delay-75">

            </div>
        </div>
    </div>
</x-app-layout>
